...
- **실제 광고 렌더링은 Toss 앱 또는 샌드박스 안에서만 동작합니다.** 일반 브라우저나 Unity Editor에서는 광고 네트워크에 도달하지 못하므로, 초기화 이후 `FailedToRender`·`NoFill`(배너) 또는 에러 콜백이 오는 것이 정상입니다.
...
1. **Toss 앱 안에서 실행 중인가?** 실제 렌더링은 Toss 앱이나 샌드박스 안에서만 됩니다. 일반 브라우저와 Editor에서는 초기화 후 실패·노필 이벤트가 오는 것이 정상입니다.
