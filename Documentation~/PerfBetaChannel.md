...
아래 레버는 **기본값 `-1`이 auto-OFF**입니다(품질 검증 게이트 미통과). 켜려면 반드시 값을 `1`로 명시해야 합니다. 오디오/텍스처 두 레버는 lossy이므로 켠 뒤 반드시 품질 검수하세요 — 프로젝트 원본은 건드리지 않고 외부화된 스트림 사본(CDN 배포본)만 교체합니다. 폰트 lazy 확장 레버는 시청각 손실은 없지만 로드 타이밍 트레이드오프(아래 표 참고)가 있어 같은 opt-in posture를 취합니다.
...
| 폰트 subset 선택 언어 lazy 확장 (실험적) | `fontSubsetLazyLanguages = 1` (기본 `-1` = auto-OFF) | 추가 필드 없음(`fontSubsetLanguages`로 선택한 언어 중 ko/la를 제외한 전부가 대상) | 시청각 손실 없음이나 **로드 타이밍 트레이드오프**: 부트 .data 폰트 크기를 줄이는 대신, 해당 문자체계 텍스트가 처음 등장한 순간부터 lazy 번들 다운로드가 완료될 때까지 그 글자들이 일시적으로 □(tofu)로 렌더된 뒤 채워짐. lazy 확장의 어느 단계든 실패하면 그 언어는 안전하게 부트 union으로 폴백(fallback-to-boot)되어 1단계(순수 boot subset) 대비 tofu 리스크가 늘지 않음 | 콘텐츠 최적화 — 폰트 CJK subset |
...
2. 위 표의 설정 위치에 해당하는 foldout/Header를 펼칩니다. 콘텐츠 최적화 레버는 Inspector에서 각 `콘텐츠 최적화 — …` Header 아래에 위치합니다.
   ...
   - **명시 활성 전용 레버 (기본 OFF)**: 스트리밍 오디오 트랜스코딩·스트림 PNG→JPEG·폰트 subset 선택 언어 lazy 확장 — 켜려면 값을 `1`로 설정
...
- **폰트 subset 선택 언어 lazy 확장(실험적, 기본 OFF)**: `fontSubsetLazyLanguages = 1`로 켜면 `fontSubsetLanguages`로 선택한 언어 중 ko/la를 제외한 전부가 부트 union 대신 별도 AssetBundle로 분리되어, 해당 문자체계 텍스트가 실제로 등장할 때만 다운로드됩니다. 로드 전에는 그 글자들이 일시적으로 □(tofu)로 보이다가 다운로드 완료 후 채워집니다 — 초기 화면(부트 직후)에 lazy 대상 언어 텍스트를 즉시 노출하는 화면이 있다면 그 화면에서 일시적 tofu가 보일 수 있으니 UX 영향을 검토하세요. lazy 확장의 어느 단계든 실패해도 해당 언어는 안전하게 부트 union으로 폴백(fallback-to-boot)되어 1단계 대비 tofu 리스크가 늘지 않습니다. 이 레버는 perf 베타 채널 ref(`#beta-perf` 또는 스냅샷 태그)로 설치했을 때를 전제로 검증되었습니다. **lazy 확장은 TMP(TextMeshPro) fallback 주입만 복구합니다** — legacy UI Text/TextMesh가 lazy 대상 폰트를 쓰는 프로젝트에서는 재수화되지 않으므로 이 레버를 끄세요.
