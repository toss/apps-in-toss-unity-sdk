...
| PlayerPrefs 레거시 origin 마이그레이션 어댑터 | `WebGLTemplates/AITTemplate/Runtime/ait-playerprefs.js` — `resolveLegacySource()`, `getPlatformLegacySource()`, `normalizeLegacyDump()`, `applyLegacyFiles()`, `tryLegacyImport()`, `__AIT_PP_LEGACY_SOURCE__` 오버라이드 훅, `state.legacyImport`/`legacyBackend`/`legacyBytes`/`legacyMs`. 마이그레이션 창 판정은 `snapshotHasScopedFile()`(매니페스트 부재 + "PlayerPrefs 0건 매니페스트"), 창 유지는 `pushScoped()`의 `remoteHasScoped` 가드(빈 매니페스트 생성 금지), 임포트 크기 상한은 `LEGACY_MAX_BYTES`/`LEGACY_MAX_B64_CHARS`. 리매핑 기준 경로는 `resolveAppDir()` — Unity는 IDBFS를 `/idbfs` **자체**에 마운트하므로(`prejs/IdbFs.js`의 `FS.mount(IDBFS, ..., '/idbfs')`) `mount.mountpoint`는 앱 디렉터리가 아니며, `/idbfs/<hash>`는 네이티브가 `main()` 안에서 만든다. `<hash>`는 빌드가 서비스되는 URL에서 유도돼 origin이 바뀌면 값이 달라지므로 옛 경로를 그대로 심으면 안 된다. 앱 디렉터리를 특정할 수 없는 부팅(후보 0개 = 이 origin 최초 실행, 또는 2개 이상)은 `legacyImport: 'skip-unknown-appdir'`로 물러나고 매니페스트를 쓰지 않아 다음 부팅에 창을 남긴다. **후보가 1개인데 그게 stale 디렉터리인 경우는 좌초 경로에 심고 창이 닫히는 알려진 위험**이며, stub을 채우기 전 선결 과제다(TODO.md P2, 테스트 C5). 플랫폼 seam(`getPlatformLegacySource()`)은 옛 origin 저장소 조회 API 스펙 미확정으로 아직 구현 없음(stub) |
