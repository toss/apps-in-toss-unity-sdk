...
namespace AppsInToss
{
    ...
    public class AITConvertCore
    {
        ...
        /// </summary>
        public static void Init(AITBuildProfile profile = null, bool isDevServerBuild = false)
        {
            AITBuildInitializer.Init(profile, isDevServerBuild);
        }
        ...
        private static AITEditorScriptObject PrepareExport(ref AITBuildProfile profile, ref string profileName, bool isDevServerBuild = false)
        {
        ...
        /// <summary>
        /// 빌드 전 에셋 최적화 검사 (배치 모드 또는 Dev Server 빌드에서는 스킵)
        /// Dev Server 경로는 반복 루프 속도가 우선이며, 검사는 Deploy/Build & Package에서 계속 수행됨.
        /// </summary>
        /// <returns>true = 빌드 진행, false = 빌드 취소</returns>
        private static bool RunPreBuildOptimizationCheck(AITEditorScriptObject editorConfig, bool isDevServerBuild = false)
        {
            if (Application.isBatchMode) return true;
            if (isDevServerBuild)
            {
                Debug.Log("[AIT] Dev Server 빌드: 에셋 최적화 검사를 건너뜁니다 (Deploy/Build & Package에서는 계속 수행됨).");
                return true;
            }
        ...
        /// Dev Server 경로 여부. true면 IL2CPP 컴파일러 구성을 Debug로 전환하고 에셋 최적화 검사를
        /// 건너뛴다 (반복 루프 속도 우선). 기본값 false는 기존 호출부의 동작을 보존한다.
        /// </param>
        /// <returns>변환 결과</returns>
        public static AITExportError DoExport(bool buildWebGL = true, bool doPackaging = true, bool cleanBuild = false, AITBuildProfile profile = null, string profileName = null, bool skipGraniteBuild = false, bool isDevServerBuild = false)
        {
            ...
            try
            {
                var editorConfig = PrepareExport(ref profile, ref profileName, isDevServerBuild);
                ...
                // 빌드 전 에셋 최적화 검사
                if (buildWebGL && !RunPreBuildOptimizationCheck(editorConfig, isDevServerBuild))
                {
                    return AITExportError.CANCELLED;
                }
        ...
        /// <param name="isDevServerBuild">
        /// Dev Server 경로 여부. DoExport와 동일한 의미 — 후속 단계에서 StartServer가 이 메서드로
        /// 전환될 때 사용된다. 기본값 false는 기존 호출부(AITDeployManager 등)의 동작을 보존한다.
        /// </param>
            ...
            bool isDevServerBuild = false)
        {
            ...
            {
                var result = DoExport(buildWebGL, doPackaging, cleanBuild, profile, profileName, skipGraniteBuild, isDevServerBuild);
