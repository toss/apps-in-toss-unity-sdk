...
namespace AppsInToss
{
    ...
    public static partial class AIT
    {
        ...
        [Preserve]
        [APICategory("Advertising")]
        public static Action AdsShowTossAdOrShoppingAds(
            Action<ShowTossAdOrShoppingAdsEvent> onEvent,
            ShowTossAdOrShoppingAdsParams options,
            Action<AITException> onError = null)
        {
#if UNITY_WEBGL && !UNITY_EDITOR
            string subscriptionId = AITCore.Instance.RegisterSubscriptionCallback<ShowTossAdOrShoppingAdsEvent>(
                onEvent,
                onError
            );
            __AdsShowTossAdOrShoppingAds_Internal(AITJsonSettings.Serialize(options), subscriptionId, "ShowTossAdOrShoppingAdsEvent");
            return () => AITCore.Instance.Unsubscribe(subscriptionId);
#else
            // Unity Editor mock implementation
            UnityEngine.Debug.Log($"[AIT Mock] AdsShowTossAdOrShoppingAds called");
            return () => UnityEngine.Debug.Log($"[AIT Mock] AdsShowTossAdOrShoppingAds cancelled");
#endif
        }

#if UNITY_WEBGL && !UNITY_EDITOR
        [System.Runtime.InteropServices.DllImport("__Internal")]
        private static extern void __AdsShowTossAdOrShoppingAds_Internal(string options, string subscriptionId, string typeName);
#endif
        /// <param name="onEvent">이벤트 콜백</param>
        /// <param name="onError">에러 콜백</param>
        /// <returns>구독 취소를 위한 Action</returns>
        [Preserve]
        [APICategory("Advertising")]
        public static Action AdsShowTossShoppingAds(
            Action<ShowTossShoppingAdsEvent> onEvent,
            ShowTossShoppingAdsParams options,
            Action<AITException> onError = null)
        {
#if UNITY_WEBGL && !UNITY_EDITOR
            string subscriptionId = AITCore.Instance.RegisterSubscriptionCallback<ShowTossShoppingAdsEvent>(
                onEvent,
                onError
            );
            __AdsShowTossShoppingAds_Internal(AITJsonSettings.Serialize(options), subscriptionId, "ShowTossShoppingAdsEvent");
            return () => AITCore.Instance.Unsubscribe(subscriptionId);
#else
            // Unity Editor mock implementation
            UnityEngine.Debug.Log($"[AIT Mock] AdsShowTossShoppingAds called");
            return () => UnityEngine.Debug.Log($"[AIT Mock] AdsShowTossShoppingAds cancelled");
#endif
        }

#if UNITY_WEBGL && !UNITY_EDITOR
        [System.Runtime.InteropServices.DllImport("__Internal")]
        private static extern void __AdsShowTossShoppingAds_Internal(string options, string subscriptionId, string typeName);
#endif
        /// <param name="onEvent">이벤트 콜백</param>
        /// <param name="onError">에러 콜백</param>
        /// <returns>구독 취소를 위한 Action</returns>
