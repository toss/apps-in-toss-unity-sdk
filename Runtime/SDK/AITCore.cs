...
namespace AppsInToss
{
    ...
    public partial class AITCore : MonoBehaviour
    {
        ...
        /// server-side receipt validation) before deciding the result. JavaScript awaits
        /// the returned Promise, so the response may be delivered after an await.
        /// </summary>
        ...
        /// <param name="callback">Callback function (Func&lt;T, Task&lt;TResult&gt;&gt;)</param>
        public void RegisterNestedCallback<TParam, TResult>(string subscriptionId, string callbackName, Func<TParam, Task<TResult>> callback)
        {
            var key = $"{subscriptionId}_{callbackName}";
            // Wrap the callback to accept a JSON string and return Task<bool>
            Func<string, Task<bool>> wrapper = async (string jsonData) => {
                try
                {
                    var param = JsonConvert.DeserializeObject<TParam>(jsonData);
                    var result = await callback(param);
        ...
        /// This method must stay synchronous/void because Unity's SendMessage invokes it
        /// by name; the actual (possibly asynchronous) callback runs fire-and-forget so the
        /// response is sent exactly once when it completes.
        /// </summary>
        public void OnNestedCallback(string jsonPayload)
        {
            ...
            if (_nestedCallbacks.TryGetValue(key, out var rawCallback)
                && rawCallback is Func<string, Task<bool>> callback)
            {
                // Fire-and-forget: DispatchNestedCallbackAsync owns all exception handling
                // and always responds exactly once, so this task never carries an
                // unobserved exception.
                _ = DispatchNestedCallbackAsync(request.RequestId, request.Data, callback);
            }
            else
            {
        ...
        /// <summary>
        /// Awaits the (possibly asynchronous) nested callback and responds to JavaScript
        /// exactly once. Returns a Task (not async void) so a faulted user callback cannot
        /// escape as an unobserved exception and crash the WebGL player.
        /// </summary>
