...
mergeInto(LibraryManager.library, {
    ...
    __AdsShowTossAdOrShoppingAds_Internal: function(options, subscriptionId, typeName) {
        var subId = UTF8ToString(subscriptionId);
        var typeNameStr = UTF8ToString(typeName);
        var optionsObj = options ? JSON.parse(UTF8ToString(options)) : {};

        if (window.__AIT_VERBOSE) console.log('[AIT jslib] AdsShowTossAdOrShoppingAds called, id:', subId, 'options:', optionsObj);

        try {
            var unsubscribe = window.AppsInToss.Ads.showTossAdOrShoppingAds({
                options: optionsObj,
                onEvent: function(data) {
                    if (window.__AIT_VERBOSE) console.log('[AIT jslib] AdsShowTossAdOrShoppingAds event:', data);
                    var payload = JSON.stringify({
                        CallbackId: subId,
                        TypeName: typeNameStr,
                        Result: JSON.stringify({
                ...
                onError: function(error) {
                    if (window.__AIT_VERBOSE) console.log('[AIT jslib] AdsShowTossAdOrShoppingAds error:', error);
                    var errorMessage = error instanceof Error ? error.message : String(error);
                    var payload = JSON.stringify({
                        CallbackId: subId,
                        TypeName: typeNameStr,
            ...
            console.error('[AIT jslib] AdsShowTossAdOrShoppingAds error:', error);
            var errorMessage = error instanceof Error ? error.message : String(error);
            var payload = JSON.stringify({
                CallbackId: subId,
                TypeName: typeNameStr,
    ...
    __AdsShowTossShoppingAds_Internal: function(options, subscriptionId, typeName) {
        var subId = UTF8ToString(subscriptionId);
        var typeNameStr = UTF8ToString(typeName);
        var optionsObj = options ? JSON.parse(UTF8ToString(options)) : {};

        if (window.__AIT_VERBOSE) console.log('[AIT jslib] AdsShowTossShoppingAds called, id:', subId, 'options:', optionsObj);

        try {
            var unsubscribe = window.AppsInToss.Ads.showTossShoppingAds({
                options: optionsObj,
                onEvent: function(data) {
                    if (window.__AIT_VERBOSE) console.log('[AIT jslib] AdsShowTossShoppingAds event:', data);
                    var payload = JSON.stringify({
                        CallbackId: subId,
                        TypeName: typeNameStr,
                        Result: JSON.stringify({
                ...
                onError: function(error) {
                    if (window.__AIT_VERBOSE) console.log('[AIT jslib] AdsShowTossShoppingAds error:', error);
                    var errorMessage = error instanceof Error ? error.message : String(error);
                    var payload = JSON.stringify({
                        CallbackId: subId,
                        TypeName: typeNameStr,
            ...
            console.error('[AIT jslib] AdsShowTossShoppingAds error:', error);
            var errorMessage = error instanceof Error ? error.message : String(error);
            var payload = JSON.stringify({
                CallbackId: subId,
                TypeName: typeNameStr,
