...
mergeInto(LibraryManager.library, {
    __appsInTossSignTossCert_Internal: function(params, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('appsInTossSignTossCert/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('appsInTossSignTossCert/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'appsInTossSignTossCert',
            eventId: eventId,
