/**
 * AppsInToss-CheckoutPayment.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: CheckoutPayment
 */

mergeInto(LibraryManager.library, {
    __checkoutPayment_Internal: function(options, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('checkoutPayment/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('checkoutPayment/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
