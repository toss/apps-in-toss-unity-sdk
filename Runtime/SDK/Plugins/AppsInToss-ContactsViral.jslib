...
mergeInto(LibraryManager.library, {
    __contactsViral_Internal: function(params, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('contactsViral/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('contactsViral/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
