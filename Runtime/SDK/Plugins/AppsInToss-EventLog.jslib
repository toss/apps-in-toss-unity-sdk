...
mergeInto(LibraryManager.library, {
    __eventLog_Internal: function(params, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('eventLog/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('eventLog/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
