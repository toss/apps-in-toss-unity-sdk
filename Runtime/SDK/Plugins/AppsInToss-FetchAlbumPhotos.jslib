/**
 * AppsInToss-FetchAlbumPhotos.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * 생성 시각: 2025-11-27T13:37:25.163Z
 * Category: FetchAlbumPhotos
 */

mergeInto(LibraryManager.library, {
    __fetchAlbumPhotos_Internal: function(options, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('fetchAlbumPhotos/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('fetchAlbumPhotos/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
