/**
 * AppsInToss-GenerateHapticFeedback.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: GenerateHapticFeedback
 */

mergeInto(LibraryManager.library, {
    __generateHapticFeedback_Internal: function(options, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('generateHapticFeedback/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('generateHapticFeedback/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'generateHapticFeedback',
            eventId: eventId,
