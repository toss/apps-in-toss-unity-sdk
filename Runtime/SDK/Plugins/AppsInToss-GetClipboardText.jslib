...
mergeInto(LibraryManager.library, {
    __getClipboardText_Internal: function(callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getClipboardText/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getClipboardText/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
