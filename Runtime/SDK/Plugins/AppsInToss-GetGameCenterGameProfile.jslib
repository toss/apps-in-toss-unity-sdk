/**
 * AppsInToss-GetGameCenterGameProfile.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: GetGameCenterGameProfile
 */

mergeInto(LibraryManager.library, {
    __getGameCenterGameProfile_Internal: function(callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getGameCenterGameProfile/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getGameCenterGameProfile/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'getGameCenterGameProfile',
            eventId: eventId,
            args: []
        }));
    },
