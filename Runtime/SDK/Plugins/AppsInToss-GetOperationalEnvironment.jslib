/**
 * AppsInToss-GetOperationalEnvironment.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: GetOperationalEnvironment
 */

mergeInto(LibraryManager.library, {
    __getOperationalEnvironment_Internal: function(callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getOperationalEnvironment/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getOperationalEnvironment/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'getOperationalEnvironment',
            eventId: eventId,
            args: []
        }));
    },
