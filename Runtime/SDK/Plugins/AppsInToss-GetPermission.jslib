/**
 * AppsInToss-GetPermission.jslib
 *
 * 이 파일은 자동 생성되었습니다.
...
mergeInto(LibraryManager.library, {
    __getPermission_Internal: function(permission, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getPermission/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getPermission/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
