/**
 * AppsInToss-GetPlatformOS.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: GetPlatformOS
 */

mergeInto(LibraryManager.library, {
    __getPlatformOS_Internal: function(callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getPlatformOS/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getPlatformOS/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
