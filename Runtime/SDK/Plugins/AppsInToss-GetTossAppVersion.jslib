...
mergeInto(LibraryManager.library, {
    __getTossAppVersion_Internal: function(callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getTossAppVersion/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getTossAppVersion/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'getTossAppVersion',
            eventId: eventId,
            args: []
        }));
    },
