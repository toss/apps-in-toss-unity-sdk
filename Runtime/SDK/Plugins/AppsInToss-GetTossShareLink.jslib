/**
 * AppsInToss-GetTossShareLink.jslib
 *
 * 이 파일은 자동 생성되었습니다.
...
mergeInto(LibraryManager.library, {
    __getTossShareLink_Internal: function(path, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getTossShareLink/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getTossShareLink/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'getTossShareLink',
            eventId: eventId,
            args: [UTF8ToString(path)]
        }));
    },
