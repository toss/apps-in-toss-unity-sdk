/**
 * AppsInToss-GetUserKeyForGame.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: GetUserKeyForGame
 */

mergeInto(LibraryManager.library, {
    __getUserKeyForGame_Internal: function(callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('getUserKeyForGame/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('getUserKeyForGame/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'getUserKeyForGame',
            eventId: eventId,
            args: []
        }));
    },
