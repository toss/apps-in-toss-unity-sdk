/**
 * AppsInToss-GrantPromotionRewardForGame.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: GrantPromotionRewardForGame
 */
...
mergeInto(LibraryManager.library, {
    __grantPromotionRewardForGame_Internal: function(options, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('grantPromotionRewardForGame/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('grantPromotionRewardForGame/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'grantPromotionRewardForGame',
            eventId: eventId,
