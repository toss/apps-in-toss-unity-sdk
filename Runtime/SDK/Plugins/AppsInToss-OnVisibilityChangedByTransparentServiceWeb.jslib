...
mergeInto(LibraryManager.library, {
    __onVisibilityChangedByTransparentServiceWeb_Internal: function(eventParams, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('onVisibilityChangedByTransparentServiceWeb/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('onVisibilityChangedByTransparentServiceWeb/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'onVisibilityChangedByTransparentServiceWeb',
            eventId: eventId,
            args: [eventParams]
        }));
    },
