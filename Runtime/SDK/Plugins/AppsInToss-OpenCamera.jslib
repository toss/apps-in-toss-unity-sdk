...
mergeInto(LibraryManager.library, {
    __openCamera_Internal: function(options, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('openCamera/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('openCamera/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
