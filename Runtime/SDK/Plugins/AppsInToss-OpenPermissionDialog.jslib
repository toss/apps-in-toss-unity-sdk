/**
 * AppsInToss-OpenPermissionDialog.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: OpenPermissionDialog
 */

mergeInto(LibraryManager.library, {
    __openPermissionDialog_Internal: function(permission, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('openPermissionDialog/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('openPermissionDialog/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'openPermissionDialog',
            eventId: eventId,
            args: [JSON.parse(UTF8ToString(permission))]
        }));
    },
