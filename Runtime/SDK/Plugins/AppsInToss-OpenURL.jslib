...
mergeInto(LibraryManager.library, {
    __openURL_Internal: function(url, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('openURL/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('openURL/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
