/**
 * AppsInToss-PermissionManagement.jslib
 *
 * 이 파일은 자동 생성되었습니다.
...
mergeInto(LibraryManager.library, {
    requestPermission: function(permission, callbackId, typeName) {
        const callback = UTF8ToString(callbackId);
        const typeNameStr = UTF8ToString(typeName);
        // @apps-in-toss/web-framework API 호출
        if (typeof window.AppsInToss !== 'undefined' && window.AppsInToss.requestPermission) {
            window.AppsInToss.requestPermission(JSON.parse(UTF8ToString(permission)))
            ...
            console.warn('window.AppsInToss.requestPermission not available');
        }
    },
