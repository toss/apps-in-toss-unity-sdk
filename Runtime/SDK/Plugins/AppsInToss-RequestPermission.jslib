/**
 * AppsInToss-RequestPermission.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: RequestPermission
 */

mergeInto(LibraryManager.library, {
    __requestPermission_Internal: function(permission, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('requestPermission/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('requestPermission/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
