...
mergeInto(LibraryManager.library, {
    __setClipboardText_Internal: function(text, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('setClipboardText/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('setClipboardText/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'setClipboardText',
            eventId: eventId,
            args: [UTF8ToString(text)]
        }));
    },
