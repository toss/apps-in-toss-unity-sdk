/**
 * AppsInToss-SetSecureScreen.jslib
 *
 * 이 파일은 자동 생성되었습니다.
 ...
 * Category: SetSecureScreen
 */

mergeInto(LibraryManager.library, {
    __setSecureScreen_Internal: function(options, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('setSecureScreen/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('setSecureScreen/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
