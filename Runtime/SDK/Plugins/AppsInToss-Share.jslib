...
mergeInto(LibraryManager.library, {
    __share_Internal: function(message, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('share/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('share/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            ...
            functionName: 'share',
            eventId: eventId,
            args: [JSON.parse(UTF8ToString(message))]
        }));
    },
