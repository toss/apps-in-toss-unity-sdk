...
mergeInto(LibraryManager.library, {
    __startUpdateLocation_Internal: function(eventParams, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('startUpdateLocation/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('startUpdateLocation/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'startUpdateLocation',
            eventId: eventId,
            args: [JSON.parse(UTF8ToString(eventParams))]
        }));
    },
