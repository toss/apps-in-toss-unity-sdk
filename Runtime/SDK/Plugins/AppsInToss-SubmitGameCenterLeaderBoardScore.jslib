 ...
 * 생성 시각: 2025-11-26T13:03:05.261Z
 * Category: SubmitGameCenterLeaderBoardScore
 */
...
mergeInto(LibraryManager.library, {
    __submitGameCenterLeaderBoardScore_Internal: function(params, callbackId, typeName) {
        ...
        // 응답 리스너 등록
        var removeResolve = emitter.on('submitGameCenterLeaderBoardScore/resolve/' + eventId, function(result) {
            removeResolve();
            removeReject();
        ...
        var removeReject = emitter.on('submitGameCenterLeaderBoardScore/reject/' + eventId, function(error) {
            removeResolve();
            removeReject();
            var payload = JSON.stringify({
                CallbackId: callback,
        ...
        webView.postMessage(JSON.stringify({
            type: 'method',
            functionName: 'submitGameCenterLeaderBoardScore',
            eventId: eventId,
            args: [JSON.parse(UTF8ToString(params))]
        }));
    },
