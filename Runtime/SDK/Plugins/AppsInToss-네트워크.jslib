...
mergeInto(LibraryManager.library, {
    getNetworkStatus: function(callbackId, typeName) {
        const callback = UTF8ToString(callbackId);
        const typeNameStr = UTF8ToString(typeName);
        // @apps-in-toss/web-framework API 호출
        if (typeof window.AppsInToss !== 'undefined' && window.AppsInToss.getNetworkStatus) {
            window.AppsInToss.getNetworkStatus()
            ...
            console.warn('window.AppsInToss.getNetworkStatus not available');
        }
    },
