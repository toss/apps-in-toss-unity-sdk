...
mergeInto(LibraryManager.library, {
    eventLog: function(params, callbackId, typeName) {
        const callback = UTF8ToString(callbackId);
        const typeNameStr = UTF8ToString(typeName);
        // @apps-in-toss/web-framework API 호출
        if (typeof window.AppsInToss !== 'undefined' && window.AppsInToss.eventLog) {
            window.AppsInToss.eventLog(JSON.parse(UTF8ToString(params)))
            ...
            console.warn('window.AppsInToss.eventLog not available');
        }
    },
