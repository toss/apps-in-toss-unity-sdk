...
mergeInto(LibraryManager.library, {
    checkoutPayment: function(options, callbackId, typeName) {
        const callback = UTF8ToString(callbackId);
        const typeNameStr = UTF8ToString(typeName);
        // @apps-in-toss/web-framework API 호출
        if (typeof window.AppsInToss !== 'undefined' && window.AppsInToss.checkoutPayment) {
            window.AppsInToss.checkoutPayment(JSON.parse(UTF8ToString(options)))
            ...
            console.warn('window.AppsInToss.checkoutPayment not available');
        }
    },
