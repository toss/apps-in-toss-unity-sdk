...
mergeInto(LibraryManager.library, {
    openURL: function(url, callbackId, typeName) {
        const callback = UTF8ToString(callbackId);
        const typeNameStr = UTF8ToString(typeName);
        // @apps-in-toss/web-framework API 호출
        if (typeof window.AppsInToss !== 'undefined' && window.AppsInToss.openURL) {
            window.AppsInToss.openURL(UTF8ToString(url))
            ...
            console.warn('window.AppsInToss.openURL not available');
        }
    },
