...
- **P2 — 이전 origin 저장소 조회 수단 확보 시 어댑터 연결**: SDK 3.x로 오면서 서빙 origin이 변경됐다(플랫폼 공지 기준). 브라우저 저장소는 origin 단위로 격리되므로, 순정 경로의 PlayerPrefs가 놓이는 IDBFS(IndexedDB)는 origin이 바뀌면 이전 데이터에 접근할 수 없다. 플랫폼이 마이그레이션 지원 방안을 검토 중이나 **구체적인 방법과 일정은 미정**이다. 우리가 필요로 하는 형태는 IndexedDB DB명 `/idbfs`, 오브젝트스토어 `FILE_DATA`의 덤프(키=파일 경로, 값=contents/mode/timestamp)이며, 이 요구는 플랫폼 측에 전달돼 있다. 이번 변경으로 레이어에 mock 주입 가능한 seam(`__AIT_PP_LEGACY_SOURCE__` 오버라이드 훅 + `getPlatformLegacySource()` stub)이 들어가 있어, 수단이 확정되면 stub 하나를 채우는 작은 통합만 남는다. **⚠️ stub을 채우기 전에 반드시 해소해야 하는 선결 과제**: 심을 경로 `/idbfs/<hash>/PlayerPrefs`의 `<hash>`는 빌드가 서비스되는 URL에서 유도돼 origin이 바뀌면 값이 달라지는데, 이 디렉터리는 Unity 네이티브가 `main()` 안에서 만들기 때문에 populate 시점(= 임포트 시점)에는 아직 없다. 그래서 `resolveAppDir()`은 로컬 엔트리 목록에서 `/idbfs/<한 세그먼트>` 후보를 찾아 **정확히 1개일 때만** 채택한다. 여기서 두 가지가 갈린다.

  - 후보 0개(이 origin 최초 실행) 또는 2개 이상 → `legacyImport: 'skip-unknown-appdir'`로 물러나고 매니페스트를 쓰지 않아 창이 열린 채 남는다. 이관은 앱 디렉터리가 생긴 **다음 부팅**으로 밀릴 뿐이라 안전하다.
  - **후보가 1개인데 그게 현재 앱 디렉터리가 아닌 경우 → 좌초 경로에 심고 창이 영구히 닫힌다.** 같은 origin에서 서빙 URL만 바뀌면(경로 버저닝 `/app/v1` → `/app/v2` 등) 옛 URL의 `/idbfs/<hashA>`는 populate로 복원되는데 현재 빌드의 `<hashB>`는 아직 없으므로, 유일 후보 검사를 `<hashA>`가 통과한다. 심은 결과가 매니페스트로 승격되면 다음 부팅은 `snapshotHasScopedFile`이 true라 `present` 분기로 빠져 재시도 기회가 사라진다. (옛 디렉터리에 PlayerPrefs가 남아 있으면 `skip-local-present`로 먼저 빠지므로, 위험한 조합은 "PlayerPrefs 없는 stale 디렉터리 1개"뿐이다.) 이 거동은 `playerprefs-adapter.test.ts`의 C5가 **위험을 고정하는 용도로** 재현해 두었다 — 통과가 곧 올바름을 뜻하지 않으며, 해소 시 기대값을 바꿔야 한다.

  현재는 `getPlatformLegacySource()`가 null stub이라 오버라이드 훅 없이는 도달하지 않으므로 잠재 상태다. 해소 방향은 populate 시점 추측을 버리고 앱 디렉터리 생성 시점(`node_ops.mkdir`)에 동기 주입하거나, `logFirstPersist`가 이미 모으는 "첫 persist 시점의 로컬 목록"처럼 **Unity가 실제로 쓴 디렉터리라는 양성 신호**에 앵커하는 것이다. 부트 크리티컬 경로를 건드리는 만큼 실제 API가 확정된 뒤에 설계를 확정한다. 어댑터를 의도적으로 얇게 유지하는 근거: IndexedDB는 웹 표준상 best-effort 저장소라 이미 좌초된 데이터를 구조하는 일의 기대값이 낮고, 가치의 본체는 앞으로의 쓰기를 IndexedDB에서 걷어내는 쪽에 있다.
